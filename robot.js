...
export function makeMetalRobot() {
  ...
  const bodyL = new THREE.Mesh(
    ...
    const bodyR = new THREE.Mesh(
      ...
      const triangleVerices = new Float32Array( [
        0, 0, bodyD/2+gap,(bodyW - bodyD)/2,bodyH/2,bodyD/2+gap,
        -(bodyW - bodyD)/2,bodyH/2,bodyD/2+gap]);
      triangleGeometry.setAttribute('position',
        new THREE.BufferAttribute(triangleVerices, 3));
...
export function makeCBRobot() {
  ...
  body.position.set((bodyW - bodyD)/2,legLen + bodyH/2,0);
  ...
  const triangleGeometry = new THREE.BufferGeometry();
      const triangleVerices = new Float32Array( [
        0, -mouthT, headD/2+gap,-mouthW/2,-(mouthT+mouthH),headD/2+gap,
